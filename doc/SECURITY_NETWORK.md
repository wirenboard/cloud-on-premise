...
> `traefik/` и `README(_EN).md` этой же ревизии репозитория.
...
(каждый контроллер получает свой сабдомен ssh/http через облако). Wildcard `*.apps`
(обязателен в сертификате и DNS, используется по требованию) — сервис-туннели
(веб-сервисы контроллеров, например Node-RED):
...
_Легенда: **красный** — публичные порты (открыты наружу), **синий** — внешние сервисы (исходящие соединения, egress); пунктир — исходящий трафик. **Метрики** уходят к WB только в бесплатной (FREE) версии; **почта** — только при явном `EMAIL_ENABLED=True` (переменная обязательна с 2.0)._
