...
object storage are not exposed; InfluxDB is reachable only through Traefik (443)
at `https://influx.your-domain.com`.
...
|---------|-------------|---------|---------|
| **traefik** | `443` (override: `TRAEFIK_EXTERNAL_PORT`) | **yes** | Web access to the cloud (HTTPS/TLS), API, frontend, agent, ssh/http proxying to tunnels |
...
| influx (InfluxDB) | — | no | Controller metrics storage; no host port published; reachable through Traefik: `https://influx.your-domain.com` (receives metrics from controllers) |
| minio | — | no | Object storage, internal network only |
| backend / agent_backend | — | no | API (`:8000`), reachable by Traefik, tunnel_auth, frontend and tunnel (webhook) |
...
| worker / worker-influx / worker-grafana / scheduler | — | no | Background tasks |
...
| DB / cache / storage / metrics | backend, worker | postgres / redis / minio / influx | — | internal network |
...
the `*.ssh.` (SSH-in-browser, the `webssh` service) and `*.http.` (controller
web UI) subdomains. In other words, **7107 must be open for inbound traffic**
from the networks where controllers are located. 7501 (dashboard) is optional
(see §7).
...
Internally, controller metrics are stored in the local **InfluxDB** (no host
port published; controllers deliver metrics through Traefik:
`https://influx.your-domain.com`).
...
|------|----------|--------|-----|
| **443** | TCP/HTTPS | operators + networks with WB controllers | Web access to the cloud; agent API `agent.*` (mTLS) and controller activation links |
...
**Do NOT expose:** the postgres / redis / influx / minio / backend:8000 /
webssh:8888 ports — they exist only inside the docker network.
...
| Docker images (install/upgrade) | `ghcr.io` + `registry-1.docker.io` / `docker.io` (postgres, redis, influx, minio, traefik) | 443 | Needed only during install/upgrade; can stay blocked the rest of the time |
