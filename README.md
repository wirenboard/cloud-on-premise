...
### 2. Кастомные логотипы и иконки

Этот шаг опционален. Если ничего не добавлять в `./branding`, интерфейс продолжит работать с логотипами и иконками Wiren Board по умолчанию.

Сервис `frontend` автоматически монтирует директорию `./branding` в контейнер как `/branding`. Если нужен кастомный брендинг, положите в эту папку файлы с фиксированными именами:

- `logo.svg` - логотип в интерфейсе
- `favicon.svg` - основной favicon в SVG
- `favicon.ico` - favicon для старых браузеров
- `favicon-192.png` - иконка 192x192 для Android/PWA
- `favicon-512.png` - иконка 512x512 для Android/PWA
- `apple-touch-icon.png` - иконка для iOS

Можно заменить только нужные файлы, сохранив их имена. 

После обновления ассетов перезапустите фронтенд:

```bash
docker compose restart frontend
```

Если браузер продолжает показывать старые иконки, выполните hard refresh или очистите его кеш.

### 3. Автоматическая инициализация и запуск
