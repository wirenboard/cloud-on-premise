...
- **У каждого пользователя должен быть корректный, уникальный email, равный логину.**
  ...
  - **админ без email** (типичный случай 1.x: `username="admin"`, `email=""`) —
    задайте `ADMIN_EMAIL` в `.env`, тогда он исправится автоматически;
...
4. **Место на диске** — не меньше 6 ГБ под новые образы.
...
```sh
# Только посмотреть конфликты (read-only):
make fix-users MODE=scan

# Применить безопасные авто-исправления (email в нижний регистр; различия только
# в регистре/пробелах; email администратора из ADMIN_EMAIL):
make fix-users MODE=auto

# Интерактивный мастер: запрашивает корректный email по каждому конфликту,
# проверяет валидность и отсутствие коллизий:
...
> Бэкап `./backups/pg-<ts>.sql.gz` — ваша страховка. Если миграция по какой-то
> причине прервётся, восстановите базу из этого дампа и повторите.
