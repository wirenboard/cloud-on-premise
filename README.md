...
### 5. Кастомный логотип, иконки и название
...
Помимо ассетов, через переменные окружения в `.env` можно переопределить название сервиса и ссылки:

- `SERVICE_NAME` — название сервиса: заголовок вкладки браузера, надпись на странице входа и упоминания в текстах интерфейса. По умолчанию — `Wiren Board Cloud`.
- `SERVICE_STATUS_URL` — ссылка на страницу статуса сервисов, показывается при долгом открытии туннеля. По умолчанию не задана — ссылка скрыта.
- `SERVICE_DOCS_URL` — ссылка на документацию на экране подключения контроллера. По умолчанию — страница Wiren Board Cloud в вики; пустое значение скрывает ссылку.

Если проект уже запущен, после замены файлов или изменения переменных перезапустите фронтенд:
