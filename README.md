...
Помимо ассетов можно переопределить название продукта и ссылки, которые фронтенд показывает в интерфейсе. Для этого раскомментируйте и заполните переменные в `.env`:
...
Если переменные не заданы, используются значения Wiren Board по умолчанию. После изменения `.env` перезапустите фронтенд.
