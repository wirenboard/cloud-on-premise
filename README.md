...
>
> Параллелизм фоновых задач рассчитан на минимальную машину. Если сервер свободен, а задачи копятся,
> его можно поднять переменными `WORKER_CONCURRENCY`, `METRICS_WORKER_CONCURRENCY`,
> `GRAFANA_WORKER_CONCURRENCY`, `EMAIL_WORKER_CONCURRENCY` (см. `.env.example`) — каждая единица
> добавляет примерно 90 МБ памяти.
...
Фоновые задачи облака разнесены по четырём очередям, у каждой свой обработчик. Значения по
умолчанию рассчитаны на минимальную конфигурацию сервера и на несколько десятков контроллеров.
...
**Сколько это стоит по памяти.** Каждая единица параллелизма — это отдельный процесс,
примерно **85 МБ**. Значения по умолчанию (4 + 3 + 3 + 2) занимают около 1.3 ГБ. Прибавка
считается просто: `+1` к любой переменной — это ещё около 85 МБ.

Ориентиры по масштабу:

| Контроллеров | Значения | Память под воркеры |
|---|---|---|
| до 20 | по умолчанию: 4 / 3 / 3 / 2 | ~1.3 ГБ |
| до 50 | 4 / 4 / 4 / 2 | ~1.5 ГБ |
| до 100 | 6 / 6 / 6 / 3 | ~2.2 ГБ (нужна рекомендуемая конфигурация сервера) |
