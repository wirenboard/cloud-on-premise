...
- **приглашение пользователя в организацию** (в два шага, так как письмо не отправляется):
  ...
  2. откройте [административную панель](#административная-панель) → раздел **Organizations** → нужная организация → блок **Invites** и скопируйте значение поля **link**. Передайте ссылку пользователю любым способом — по ней он зарегистрируется без подтверждения почты.
