...
Верхней границы нет — ограничение только память сервера. Для сравнения: облако Wiren Board
работает с параллелизмом 10 на воркер и обслуживает тысячи контроллеров.
...
> ⚠️ В 2.0 переменная `EMAIL_ENABLED` обязательна: без явного `True`/`False` стек не стартует.
