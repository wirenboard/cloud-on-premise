...
| Переменная | По умолчанию | За что отвечает очередь | Когда поднимать |
|---|---|---|---|
| `WORKER_CONCURRENCY` | 4 | Общие задачи: активация контроллеров, обслуживание туннелей, лицензии | Много контроллеров подключается и отключается одновременно |
| `METRICS_WORKER_CONCURRENCY` | 3 | Метрики: рассылка конфигурации сбора, роли и политики хранения в TimescaleDB | Растёт число контроллеров, метрики появляются с задержкой |
| `GRAFANA_WORKER_CONCURRENCY` | 3 | Дашборды и пользователи Grafana | Много организаций и пользователей, дашборды создаются медленно |
| `EMAIL_WORKER_CONCURRENCY` | 2 | Отправка писем: приглашения, сброс пароля, оповещения | Массовые рассылки приглашений или много правил оповещения |
...
> Оценки в таблице выведены из расхода памяти на процесс и из настроек нашего облака; пропускная
> способность зависит от конкретной нагрузки, поэтому ориентируйтесь в первую очередь на длину
> очередей у себя.
