...
Пока доступна только бесплатная версия на 100 контроллеров, которую можно использовать в личных и коммерческих целях. В этой версии включена обязательная отправка обезличенных метрик к нам на сервер, что именно отправляется смотрите в бэкенде инстанса «On Premise» → «Метрики».
